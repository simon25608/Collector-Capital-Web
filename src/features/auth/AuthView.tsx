...
export function AuthView({ setView }: AuthViewProps) {
  ...
  return (
    <main className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 flex flex-col md:flex-row items-stretch gap-8 sm:gap-12 min-h-screen pt-20 sm:pt-24">
      {/* Background Layer */}
      ...
      <section className="hidden md:flex flex-1 flex-col justify-between p-12 rounded-3xl bg-surface-container-low relative overflow-hidden">
        <div className="absolute inset-0 opacity-20 pointer-events-none">
          ...
          >
            ...
            ></path>
            <path
              d="M0,150 Q120,100 240,150 T400,150"
              fill="none"
              stroke="#4edea3"
              strokeWidth="0.5"
            ></path>
            <path
              d="M0,200 Q80,250 160,200 T400,200"
              fill="none"
              stroke="#4edea3"
              strokeWidth="0.5"
            ></path>
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-5xl lg:text-6xl font-extrabold leading-[1.1] tracking-tight text-on-surface mb-6">
            {t("auth.joinTitle")}{" "}
            <span className="text-primary">{t("auth.joinHighlight")}</span>{" "}
            {t("auth.joinSuffix")}
          </h1>
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex -space-x-3">
              <img
                ...
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuA5wk0qZjJWKmdGBqDr3rulheyavN48-xEKVRYjXG00UBG3YFm--oPCyk8UJF255UAPeT_Fq_s5EfepjYlZHM4KVJMBncC53rWpj0I0eAbPeeoAyUUwFhF7hPhuMhw5U4w2kZT2Fr_5AT6pMhjLp9j1cnQB3oQVCIfVEO0tbkV3s1BbHmEJNHDZbc7BFgEVLKDzULxyag6CyWkB842uMUX71bIEuP47ZWTn_FfaZWeWwltwv_OxXlK184Mu1AMCH08IVourzYV27kY"
              />
              <img
                alt="User"
                className="w-10 h-10 rounded-full border-2 border-surface-container-low"
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuAhloHRIe7u76z4-cWCwxaX-09ODDwhT50-9A64laByYDusojUft9b9b7cfMY12pJMVzLf9tK5J_kjBTrPHZEaZ8xU0nq-zq-eqf0lIaSgnEUNEEV7ESFddpqwtalafNM126zlBfsntoYC3Se_GYCkZ1k3Rq6EiBXNBtCmNLtcTT95WmLefXTMiRkL0geHMVdnn_8ZuW-IP5MOXyR5IllLN6OWGF2WFk5G5_fpvzeC9fgXNVYS-R0MMMwQkTaE7stOsetmkA-I6JWM"
              />
              <img
                alt="User"
                className="w-10 h-10 rounded-full border-2 border-surface-container-low"
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuDxrg8WMgpxHKcOF4iFLBxJd0SPqb0mCEOlRMO5Qt0W-MbPeFG8psszQ2ddOYjvONpAm8Vf3SeBLdXlUkzixTTLa5TUJsFKxrwVeka02BToQjUrF5h1TJQGlTfQAIT4jPR85tOorJ_bWO74HH-jPMbVQx0GgfIELBxFtY6cGdW4-GdRDqfWdemwbhfRWjho-FRzt5XfbFUoiP9ePoGQ1n7rgC7ES5hRiJ-itugkXpaH0ZpUeJlfUS80KDtMtSBu58pmy6zcKzDTgyo"
              />
            </div>
          ...
          {success ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-4 mb-8">
                <button className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-surface-container-high hover:bg-surface-container-highest transition-colors border border-outline-variant/10">
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path
                      d="M12.48 10.92v3.28h7.84c-.24 1.84-.92 3.32-2.12 4.4-1.32 1.32-3.4 2.68-6.92 2.68-5.76 0-10.36-4.64-10.36-10.36s4.6-10.36 10.36-10.36c3.12 0 5.4 1.2 7.08 2.84l2.32-2.32c-2.44-2.32-5.64-3.64-9.4-3.64-7.84 0-14.28 6.44-14.28 14.28s6.44 14.28 14.28 14.28c4.24 0 7.44-1.4 9.88-3.92 2.52-2.52 3.32-6.04 3.32-8.76 0-.84-.08-1.44-.16-2.08h-13.04z"
                      fill="#EA4335"
                    ></path>
                  </svg>
              ...
              <div className="relative flex py-5 items-center mb-4">
                ...
                <span className="flex-shrink mx-4 text-on-surface-variant text-xs font-medium uppercase tracking-widest">
                  {isLogin ? t("auth.orEmailLogin") : t("auth.orEmail")}
                </span>
                <div className="flex-grow border-t border-outline-variant/15"></div>
              </div>
              ...
              <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                ...
                {!isLogin && (
                  <div>
                    ...
                    />
                    {errors.fullName && (
                      <p className="text-error text-[10px] mt-1 ml-1 uppercase tracking-wider font-bold">
                        {t("auth.required")}
                      </p>
                    )}
                  </div>
                  ...
                  {!isLogin && (
                    <>
                      <div className="mt-3 flex gap-1.5 px-1">
                        <div
                          className={`h-1 flex-1 rounded-full transition-all duration-500 ${passwordStrength >= 1 ? strengthColor : "bg-surface-bright"}`}
                        ></div>
                        <div
                          className={`h-1 flex-1 rounded-full transition-all duration-500 ${passwordStrength >= 2 ? strengthColor : "bg-surface-bright"}`}
                        ></div>
                        <div
                          className={`h-1 flex-1 rounded-full transition-all duration-500 ${passwordStrength >= 3 ? strengthColor : "bg-surface-bright"}`}
                        ></div>
                        <div
                          className={`h-1 flex-1 rounded-full transition-all duration-500 ${passwordStrength >= 4 ? strengthColor : "bg-surface-bright"}`}
                        ></div>
                      </div>
                      <p
                        className={`text-[10px] uppercase tracking-wider font-semibold mt-2 ml-1 transition-colors duration-500 ${passwordStrength >= 1 ? strengthColor.replace("bg-", "text-") : "text-primary"}`}
                      >
